    ...
    def transcribe_2(self):
        ...
        with mp_hands.Hands(min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
            while cap.isOpened():
                ...
                image.flags.writeable = False
                results = hands.process(image)
                detection_result = self.handLandmarker.detect_landmark_from_data(image)
                image.flags.writeable = True
                image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
                if (len(detection_result.hand_landmarks) > 0):
                    ...
                    mp_drawing.draw_landmarks(image, results.multi_hand_landmarks.landmarks[0], mp_hands.HAND_CONNECTIONS)
                    result = classifier.predict(data)
                    wrist_landmark = detection_result.hand_landmarks[0][mp_hands.HandLandmark.WRIST]
                    h, w, c = image.shape
                    wrist_coords = (int(wrist_landmark.x * w),int(wrist_landmark.y * h))
                    cv2.putText(image, result, wrist_coords,cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
                    cv2.imshow('ASL Transcriber', image)
                else:
                    cv2.imshow('ASL Transcriber', image)              
                if cv2.waitKey(5) & 0xFF == 27:
